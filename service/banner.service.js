...
export const bannerService = {
  async generate(nombre, apellido, fechaMezclada) {
    ...
    try {
      ...
      await activePage.type("#name", String(`${nombre} ${apellido}`), {
        delay: 30,
      });
      await activePage.type('input[placeholder="Día"]', String(day));
      await activePage.type(
        'input[placeholder="Mes (solo texto)"]',
        String(monthName)
      );
      ...
      await activePage.waitForSelector(downloadButtonSelector, {
        visible: true,
      });
      ...
      for (let i = 0; i < 20; i++) {
        ...
        const newFiles = filesAfter.filter(
          (f) => !filesBefore.includes(f) && !f.endsWith(".crdownload")
        );
      ...
      const finalFileName = `cumple_${nombre.replace(
        /\s/g,
        "_"
      )}_${Date.now()}.png`;
      fs.renameSync(
